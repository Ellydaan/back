  ...
  <div class="bg-gray-800 p-6 rounded-lg shadow-lg mx-auto mb-6">
    <div class="space-y-4">
      <div class="flex items-start">
        ...
        <div class="ml-4">
          <h3 class="text-lg font-medium text-white">Delete User Account</h3>
          <p class="mt-2 text-sm text-gray-300">
            Once you delete the account for <span class="font-bold">{{ username }}</span>, all data
        ...
        <div class="space-y-4">
          ...
          <div class="mt-3">
            <p class="text-sm text-gray-300">
              Please type the username <span class="font-bold">{{ username }}</span> to confirm the
              deletion of this account. This action cannot be undone.
            </p>
            <input
          ...
          <div class="mt-5 flex justify-end space-x-3">
            ...
            <button
              @click="handleDelete"
              :disabled="enteredUsername !== username"
            ...
            <button @click="handleDelete"
                    class="text-gray-400 hover:text-red-400 transition-all duration-300 transform hover:rotate-90"
            >
...
const handleDelete = async () => {
  try {
    if (enteredUsername.value !== props.username) {
      error.value = 'Username does not match'
      return
    }

    console.log('Deleting account for', props.username)
    ...
    error.value = ''
    router.push('/teams') // Redirect after deletion

  } catch (err) {
    error.value = 'An error occurred while deleting the account'
  }
}
</script>
